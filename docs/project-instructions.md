...
- **Phase 2 (compute nodes via hand-rolled Ansible)** — **active.** Design is done
  (VXLAN self-service networking, the staged Ansible approach); execution is the next
  work. Open items are tracked in [project-plan.md](project-plan.md) and
...
- **Stick to the established stack:** AlmaLinux 9, OpenStack 2025.1, Ceph Squid via
  `cephadm`, Linux bridge mechanism driver, VXLAN self-service tenant networking.
